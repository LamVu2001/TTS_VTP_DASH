...
def render(file_id: str):
    ...
    # Hàm trợ giúp tạo mệnh đề SQL IN (...) an toàn chống SQL Injection cho danh sách chọn
    ...
    with c_odr_chart:
        ...
        try:
            where_chart_conds = ["PTC = 1", "clean_date_ptc IS NOT NULL"]
            
            c_tinh = sql_in_clause("tinh_phat", st.session_state.f_tinh)
            if c_tinh: where_chart_conds.append(c_tinh)

            c_kh = sql_in_clause("ma_khgui", st.session_state.f_kh)
            if c_kh: where_chart_conds.append(c_kh)

            c_bc = sql_in_clause("ma_buucuc_phat", st.session_state.f_bc)
            if c_bc: where_chart_conds.append(c_bc)

            c_tuyen = sql_in_clause("tuyen", st.session_state.f_tuyen)
            if c_tuyen: where_chart_conds.append(c_tuyen)

            c_ld = sql_in_clause("ma_dv_viettel", st.session_state.f_ld)
            if c_ld: where_chart_conds.append(c_ld)

            c_tl = sql_in_clause("nhom_trong_luong", st.session_state.f_tl)
            if c_tl: where_chart_conds.append(c_tl)
