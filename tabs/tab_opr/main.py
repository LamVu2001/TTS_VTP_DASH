...
def render(file_id: str):
   ...
   # 1. KHỞI TẠO SESSION STATE BỘ LỌC
    ...
    for tinh, bc, ngay, tuan, thang, sl in tree_raw_data:
        if tinh not in tree_struct_opr:
            tree_struct_opr[tinh] = {
                ...
                'weeks': {w: 0 for w in sorted_weeks_raw},
    ...
    for idx_tinh, (tinh_name, tinh_data) in enumerate(tree_struct_opr.items()):
        ...
        tinh_week_tds = "".join([f"<td>{tinh_data['weeks'].get(w, 0):,.0f}</td>" for w in sorted_weeks_raw])
        ...
        wk_prev_tinh = tinh_data['weeks'].get(sorted_weeks_raw[-2], 0)
        wk_cur_tinh = tinh_data['weeks'].get(sorted_weeks_raw[-1], 0)
        ...
        for bc_name, bc_data in tinh_data['bcs'].items():
            ...
            bc_week_tds = "".join([f"<td>{bc_data['weeks'].get(w, 0):,.0f}</td>" for w in sorted_weeks_raw])
            ...
            wk_prev_bc = bc_data['weeks'].get(sorted_weeks_raw[-2], 0)
            wk_cur_bc = bc_data['weeks'].get(sorted_weeks_raw[-1], 0)
    ...
    <table class="matrix-table">
        <thead>
            ...
            <tr>
                ...
                <th>W{sorted_weeks_raw[0]}</th><th>W{sorted_weeks_raw[1]}</th><th>W{sorted_weeks_raw[2]}</th><th>W{sorted_weeks_raw[3]}</th><th>W{sorted_weeks_raw[4]}</th><th style="color: #ff5252;">WoW</th>
                <th>Tháng {sorted_months[0]}</th><th>Tháng {sorted_months[1]}</th><th style="color: #ff5252;">MoM</th>
            </tr>
        </thead>
        <tbody>
