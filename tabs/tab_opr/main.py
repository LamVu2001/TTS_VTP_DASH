...
def render(file_id: str):
   ...
   # 1. KHỞI TẠO SESSION STATE BỘ LỌC
    ...
    # MA TRẬN CHẤT LƯỢNG
    st.markdown('<div style="font-size:20px; font-weight:bold; color:#111; border-left:4px solid #c62828; padding-left:8px; margin-top:5px; margin-bottom:8px;">CHẤT LƯỢNG KHÂU THU</div>', unsafe_allow_html=True)
    ...
    # 4. TRUY VẤN TỶ LỆ CHO DÒNG TỔNG
    rate_dung_days = []
    rate_dung1_days = []
    for d in sorted_days_sql:
        if d == "1970-01-01":
            rate_dung_days.append(0.0)
            rate_dung1_days.append(0.0)
            continue
    ...
    for tinh, bc, ngay, tuan, thang, sl in tree_raw_data:
        ...
        if ngay in bc_node['days']: bc_node['days'][ngay] += sl
    ...
    for idx_tinh, (tinh_name, t_data) in enumerate(tree_struct_opr.items()):
        ...
        # Vòng lặp render các Bưu cục trực thuộc
